...
def parse_args():
    parser = argparse.ArgumentParser(description="Delete a user from ZKTeco device")
...
def main():
    ...
    try:
        ...
        matched_user_id = normalize_identifier(getattr(matched, "user_id", ""))
        matched_uid = int(getattr(matched, "uid", 0))

        conn.delete_user(uid=matched_uid)

        remaining_users = conn.get_users() or []
        still_exists = any(
            int(getattr(user, "uid", 0)) == matched_uid
            or normalize_identifier(getattr(user, "user_id", "")) == matched_user_id
            for user in remaining_users
        )

        if still_exists:
            raise RuntimeError(f"Failed to delete user {target_user_id}.")
            ...
            json.dumps(
                {
                    ...
                    "deleted_user_id": matched_user_id or target_user_id,
                    "deleted_uid": matched_uid,
                },
                ensure_ascii=True,
            )
        )
